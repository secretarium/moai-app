...
const PLUGIN_LIST = !fs.existsSync(YARNRC_YML_PATH) ? [] : fs.readFileSync(YARNRC_YML_PATH, 'utf-8')
    ...
    .map(line => line.match(/.*plugin-(.*?)\.cjs/)[1]);
...
try {
    ...
    // Only catch to avoid crash
    // It is valid to continue
}
...
if (CURRENT_VERSION !== REQUESTED_VERSION) {
    ...
    console.log('Downloading Yarn Berry...');
...
for (const plugin of PLUGIN_LIST) {
    ...
    if (!fs.existsSync(pluginPath)) {
        ...
        console.log(`Downloading Yarn plugin ${plugin}...`);
